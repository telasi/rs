...
public class C {
  ...
  static int INSPECTOR_ID; // = 150; -- გადმოეცემა რეესტრის ჩამოსატვირთ
  // ACTIVIT-ში
  ...
  static final String HOST = "92.241.77.36";
  ...
  public static void uploadReester(Activity context, int reesterId, int marshNUMB) {
    ((Waiting) context).setWait(true);
  ...
  private static class ReesterLoadingTask extends AsyncTask<String, Integer, Void> {
    ...
    protected Void doInBackground(String... params) {
      ...
      try {
        ...
        // String route_tarigi = reester.getCycleDate().toString();
        // 03-feb-2014
        ...
        for (int i = 0; i < reester.getItems().length; i++) {
          ...
          // double prevReading =
          // item.getReading().getPreviousReading();
          ...
          String notes = item.getReading().getNote(); // 2014-20-05 ""
          // Sevcvli mere
          // namdvili
          // mniSvnelobit
          // roca
          ...
          // dimitri magis
          // chasmaa
          Integer reading_stats = item.getReading().getErrorCode(); // es
          // ori
          // dimitrim
          // unda
          // gadmomces
          Integer noteID = item.getReading().getNoteId(); // SeniSvnis
          // kodi
          ...
          db.addRec(seq, // int P_ROWNUM,
              ...
              axali_chveneba, notes, // 2014-20-05
              noteID, // 2014-20-05
  ...
  private static class ResterUploadTask extends AsyncTask<String, Integer, Void> {
    ...
    protected Void doInBackground(String... params) {
      ...
      String where1 = DB.COLUMN_MARSH_NUM + "=" + marshNumb1; // davamater
      // 2014 09 -
      // 17
      // ??????????????????????????????????????????????????????????????????????????????????????????????
      ...
      try {
        ...
        // c = db.getAllData("1=1"); // kursoris Secvla pirobiT
        // davakomentare 2014 09 - 17
        ...
        if (c.moveToFirst()) {
          ...
          do {
            ...
            reading.setErrorCode(c.getInt(ridstatusInd)); // NEW! //
            // wakitxvis
            // statusi
            // wiTeli
            // yviteli
            ...
            reading.setNote(c.getString(noteInd)); // NEW! //
            // teqstiani
            // Setyobineba
